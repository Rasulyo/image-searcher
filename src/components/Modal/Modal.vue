      ...
      <div class="modal">
        <div class="galery-info">
            <div class="header-content">
                <div class="avatar">
                    <img :src="galery?.user.profile_image.small" alt="User avatar" />
                    <div class="info">
                        <h3>{{ galery?.user.name}}</h3>
                        <h4>{{ "@" + galery?.user.instagram_username }}</h4>
                    </div>
                </div>
                <div class="buttons">
                    <button :class="[isFavorite ? 'active-fill' : 'btn']" @click="addToFavorites()">
          ...
          <img class="gallery-img" :src="galery?.urls.regular" alt="Galery image" />
        </div>
      </div>
    </VueFinalModal>
  </template>
...
export default {
    ...
    setup(){
        ...
        const galery = computed(() => {
            return store.getters.getActiveImage
        })

        const addToFavorites = () => {
            store.dispatch('addImageToStorage', galery.value)
            isFavorite.value = !isFavorite.value;
        }
        const downloadImage = () => {
            const downloadLink = galery.value.links.download; 
